...
exports.loginUser = async (req, res) => {
    ...
    try {
        ...
        if (user.activeSession && user.activeSession.token) {
            // Optionally, verify the existing token to see if it's still valid
            try {
                jwt.verify(user.activeSession.token, process.env.JWT_SECRET);
                // If token is valid, respond with conflict
                return res.status(409).json({
                    msgKey: 'errors.session_conflict',
                    activeSession: user.activeSession
                });
            } catch (jwtError) {
                // If token is invalid/expired, clear it and proceed with new login
